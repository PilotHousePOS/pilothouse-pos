...
const ABBREVS = {
  ...
  'cllr': 'collar', 'lsh': 'leash', 'harn': 'harness', 'bcn': 'bacon',
};
...
function scoreMatch(supply, upc) {
  ...
  const sDim = extractDimension(supply.name);
  const uDim = extractDimension(upc.name_original);
  if (sDim && uDim && sDim !== uDim) return 0;
...
async function main() {
  ...
  console.log('Total UPCs:', allUpcs.length);
  ...
  console.log('Used UPCs:', usedUpcs.size);
  ...
  for (const supply of toProcess) {
    ...
    processed++;
    if (processed % 500 === 0) {
      console.log('Processed', processed, '/', toProcess.length, '- found', matches.length, 'matches');
    }
  }
  
  matches.sort((a, b) => b.score - a.score);
  console.log('\nTotal matches found (>= 70%):', matches.length);
  
  if (matches.length > 0) {
    fs.writeFileSync('scripts/all_new_matches.json', JSON.stringify(matches, null, 2));
    console.log('Saved to scripts/all_new_matches.json');
    
    // Score breakdown
    const ranges = {
      '100%': matches.filter(m => m.score === 1.0).length,
      '90-99%': matches.filter(m => m.score >= 0.90 && m.score < 1.0).length,
      '80-89%': matches.filter(m => m.score >= 0.80 && m.score < 0.90).length,
      '70-79%': matches.filter(m => m.score >= 0.70 && m.score < 0.80).length,
    };
    console.log('\nBy score:');
    Object.entries(ranges).forEach(([r, c]) => console.log('  ' + r + ': ' + c));
  }
