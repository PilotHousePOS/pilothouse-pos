...
function scoreMatch(supply, upc) {
  ...
  // Dimension must match if both have it
...
async function main() {
  ...
  const newUpcs = allUpcs.filter(u => u.source === 'expanded_sources');
  ...
  const availableNew = newUpcs.filter(u => !usedUpcs.has(u.upc));
  console.log('Available (not used):', availableNew.length);
  ...
  console.log('Unmatched supplies:', toProcess.length);
  ...
  if (matches.length > 0) {
    fs.writeFileSync('scripts/new_source_matches.json', JSON.stringify(matches, null, 2));
    
    console.log('\nTop 20 samples:');
    matches.slice(0, 20).forEach((m, i) => {
      console.log((i+1) + '. [' + (m.score*100).toFixed(0) + '%] ' + m.brand);
