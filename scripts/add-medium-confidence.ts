...
// Load the complete UPC database
...
async function main() {
  console.log('=== Adding Medium Confidence Matches (70-89%) ===\n');
  ...
  const MEDIUM_MIN = 0.70;
  const MEDIUM_MAX = 0.90;
  
  const mediumMatches: Array<{
    productId: number;
    productName: string;
    upc: string;
    upcName: string;
  ...
  for (const [brand, brandProducts] of productsByBrand) {
    ...
    for (const product of brandProducts) {
      ...
      for (const upc of brandUpcs) {
        ...
        if (score >= MEDIUM_MIN && score < MEDIUM_MAX && (!bestMatch || score > bestMatch.score)) {
  ...
  console.log(`Medium confidence matches (70-89%): ${mediumMatches.length}`);
  ...
  console.log(`Applied ${applied} medium-confidence SKUs`);
  
  // Append to permanent matches file
  const existingMatches = JSON.parse(
    fs.readFileSync('.local/state/memory/permanent_upc_matches.json', 'utf-8')
  );
  
  const newMatches = mediumMatches.map(m => ({
  ...
  mediumMatches.slice(0, 10).forEach(m => {
