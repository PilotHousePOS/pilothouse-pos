...
for (const line of lines) {
  ...
  for (const upc of upcEntries) {
    if (Math.abs(prodNorm.length - upc.norm.length) > prodNorm.length * 0.35) continue;
    
    const sim = similarity(prodNorm, upc.norm);
    if (sim > bestSim && sim >= 0.80) {
      // Strict size verification for 80-85% matches
  ...
  processed++;
  if (processed % 300 === 0) console.log(`Processed ${processed}...`);
}

console.log(`\nTotal: ${matches.length} matches (80%+ threshold)`);

fs.writeFileSync('/tmp/fuzzy_80_updates.sql', 
  matches.map(m => `UPDATE supplies SET sku = '${m.sku}' WHERE id = ${m.id};`).join('\n'));
console.log('SQL written to /tmp/fuzzy_80_updates.sql');

console.log('\nSample 80-85% matches:');
matches.filter(m => m.sim < 0.85).slice(0, 15).forEach(m => 
