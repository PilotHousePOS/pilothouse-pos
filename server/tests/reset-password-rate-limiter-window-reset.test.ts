/**
 * Integration tests: authLimiter window reset for /api/auth/reset-password
 *
 * The authLimiter (windowMs: 15 min, max: 15) is mounted via
 *   app.use('/api/auth/reset-password', authLimiter)
 * in routes.ts.  This test confirms:
 *   1. The 16th POST to /api/auth/reset-password returns 429.
 ...
 *   UNAUTHENTICATED_NO_SLUG_ALLOWLIST (e.g. /auth/reset-password), tenantMiddleware
...
beforeAll(async () => {
  ...
  // Without this, tenantMiddleware returns 400 for /api/auth/reset-password
...
describe("authLimiter — /api/auth/reset-password: 16th request returns 429, window reset restores access", () => {
  it(
    ...
    async () => {
      ...
      // Send MAX_ATTEMPTS reset-password requests with a bogus token.
      // The authLimiter is mounted at /api/auth/reset-password — the same path
      // as the real handler — so each request is counted.  Responses before the
      // budget is exhausted must NOT be 429 (they will be 400 for invalid token).
      for (let i = 0; i < MAX_ATTEMPTS; i++) {
        const res = await agent
          .post("/api/auth/reset-password")
          .send({
            token: `bogus-reset-token-${i}`,
      ...
      const blockedRes = await agent
        .post("/api/auth/reset-password")
        .send({
          token: "bogus-reset-token-blocked",
          newPassword: "NewPassword1!",
        });
      ...
      const afterRes = await agent
        .post("/api/auth/reset-password")
        .send({
          token: "bogus-reset-token-after-rollover",
          newPassword: "NewPassword1!",
        });

      // The critical assertion: the window reset must allow new requests through.
      // The authLimiter is mounted at /api/auth/reset-password where the real
