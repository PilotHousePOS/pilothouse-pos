/**
 * Integration tests: authLimiter window reset for /api/auth/change-password
 *
 * The authLimiter (windowMs: 15 min, max: 15) is mounted via
 *   app.use('/api/auth/change-password', authLimiter)
 * in routes.ts.  This test confirms:
 *   1. The 16th POST to /api/auth/change-password returns 429.
 ...
 *   UNAUTHENTICATED_NO_SLUG_ALLOWLIST (e.g. /auth/change-password), tenantMiddleware
...
beforeAll(async () => {
  ...
  // Without this, tenantMiddleware returns 400 for /api/auth/change-password
...
describe("authLimiter — /api/auth/change-password: 16th request returns 429, window reset restores access", () => {
  it(
    ...
    async () => {
      ...
      // Send MAX_ATTEMPTS change-password requests with bogus credentials.
      // The authLimiter is mounted at /api/auth/change-password — the same path
      // as the real handler — so each request is counted.  Responses before the
      // budget is exhausted must NOT be 429 (they will be 401 or 400 for invalid
      // credentials/body).
      for (let i = 0; i < MAX_ATTEMPTS; i++) {
        const res = await agent
          .post("/api/auth/change-password")
          .send({
            currentPassword: `bogus-current-password-${i}`,
      ...
      const blockedRes = await agent
        .post("/api/auth/change-password")
        .send({
          currentPassword: "bogus-current-password-blocked",
          newPassword: "NewPassword1!",
        });
      ...
      const afterRes = await agent
        .post("/api/auth/change-password")
        .send({
          currentPassword: "bogus-current-password-after-rollover",
          newPassword: "NewPassword1!",
        });

      // The critical assertion: the window reset must allow new requests through.
      // The authLimiter is mounted at /api/auth/change-password where the real
