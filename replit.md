...
## Overview
The Animal House Pet Store is a mobile-first e-commerce platform specializing in exotic reptiles, pet supplies, and grooming services. It aims to provide a premium online shopping experience, efficient service booking, robust inventory management, and comprehensive administrative tools. The project's vision is to establish market leadership and customer trust in the exotic pet market, leveraging AI for operational and user experience enhancements.
...
## System Architecture
The Animal House Pet Store is a full-stack, mobile-first web application designed with a dark, bold, high-contrast UI/UX, featuring Amazon-style image carousels. Authentication is JWT-based, implementing a three-tier role-based access control system. The frontend is built using React, Vite, TypeScript, Tailwind CSS, shadcn/ui, Wouter for routing, and TanStack Query for state management. The backend is developed with Express.js and TypeScript, integrating with PostgreSQL and Drizzle ORM. Key features include automated product categorization, live animal detection, category cleanup, smart abbreviation expansion, and automated brand extraction. Real-time alerts are supported via Web Push Notifications using VAPID keys.
